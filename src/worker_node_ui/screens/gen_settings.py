...
from src.worker_node_ui.styles.settings_ui_py.genset_ui import Ui_Gensettings
...
class General_Setting(QWidget):
    def __init__(self, controller, parent=None):
        super().__init__(parent)
        self.controller = controller
        self.ui = Ui_Gensettings()
        self.ui.setupUi(self)

        self.setWindowFlags(
            QtCore.Qt.FramelessWindowHint | QtCore.Qt.WindowStaysOnTopHint | QtCore.Qt.Dialog)
    ...
    def open_edit_profile(self):
        if self.controller:
            self.hide() 
            self.controller.show_edit_settings(parent=self)
